...
import { ensureAdminTable, getTursoClient, normalizeAuthIdentifier, ADMIN_ACCOUNT_TABLE } from '~/server/utils/turso'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await db.execute({
    sql: `SELECT identifier, identifier_type, created_at, updated_at FROM ${ADMIN_ACCOUNT_TABLE} WHERE identifier_normalized = ? LIMIT 1`,
    args: [normalized]
  })
