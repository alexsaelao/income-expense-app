...
export default defineEventHandler(async (event) => {
  ...
  const existing = await db.execute({
    sql: `
      ...
      LIMIT 1
    `,
    args: [ownerKey, transactionId]
  })

  const row = existing.rows[0] as { created_at?: string | null } | undefined
  if (!row) {
  ...
  await db.execute({
    sql: `
      ...
      SET type = ?, wallet_id = ?, to_wallet_id = ?, currency = ?, amount = ?, exchange_rate = ?, category = ?, note = ?, transaction_date = ?, company = ?, counterparty = ?, loan_direction = ?, updated_at = ?
      WHERE owner_key = ? AND transaction_id = ?
    `,
    args: [
      payload.type,
